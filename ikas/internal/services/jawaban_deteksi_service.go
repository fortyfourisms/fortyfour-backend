...
type JawabanDeteksiService struct {
	...
	pertanyaanRepo repository.PertanyaanDeteksiRepositoryInterface
...
func (s *JawabanDeteksiService) Create(req dto.CreateJawabanDeteksiRequest, userRole string, userPerusahaanID string) (string, error) {
	...
	if s.cache != nil {
		s.cache.Delete(fmt.Sprintf("%s%s", cache.CacheKeyPrefixJawabanDeteksi, req.IkasID))
		s.cache.Delete(fmt.Sprintf("%s%s", cache.CacheKeyPrefixDeteksi, req.IkasID))
	}
...
func (s *JawabanDeteksiService) GetByUUID(uuid string, userRole string, userPerusahaanID string) (*dto.JawabanDeteksiResponse, error) {
	if uuid == "" {
...
func (s *JawabanDeteksiService) GetByIkasID(ikasID string, userRole string, userPerusahaanID string) (*dto.UnifiedJawabanDeteksiResponse, error) {
	...
	cacheKey := fmt.Sprintf("%s%s", cache.CacheKeyPrefixJawabanDeteksi, ikasID)
	if s.cache != nil {
		cachedData, err := s.cache.Get(cacheKey)
		if err == nil && cachedData != "" {
			var data dto.UnifiedJawabanDeteksiResponse
	...
	if len(data) < totalPertanyaan {
		...
		if err == nil && len(bufferData) > 0 {
			...
			for _, d := range data {
				exists[d.PertanyaanDeteksi.ID] = true
			}
			for _, b := range bufferData {
				if !exists[b.PertanyaanDeteksi.ID] {
					data = append(data, b)
				}
			}
			isDraft = true
	...
	response := &dto.UnifiedJawabanDeteksiResponse{
		Data:                 data,
		Count:                len(data),
		IsDraft:              isDraft,
	...
	if s.cache != nil {
		go func(key string, dataToCache *dto.UnifiedJawabanDeteksiResponse) {
...
func (s *JawabanDeteksiService) GetByPertanyaan(pertanyaanID int, userRole string, userPerusahaanID string) ([]dto.JawabanDeteksiResponse, error) {
	if pertanyaanID <= 0 {
...
func (s *JawabanDeteksiService) Update(uuid string, req dto.UpdateJawabanDeteksiRequest, userID string, userRole string, userPerusahaanID string) (string, string, error) {
	if uuid == "" {
	...
	if s.ikasSvc != nil {
		...
		if carryErr == nil && newIkasID != existing.IkasID {
			newJawabanID, errId := s.repo.GetIDByIkasAndPertanyaan(newIkasID, existing.PertanyaanDeteksi.ID)
			if errId == nil && newJawabanID > 0 {
				...
				if errCloned == nil {
					for _, d := range clonedData {
						if d.PertanyaanDeteksi.ID == existing.PertanyaanDeteksi.ID {
							uuid = d.UUID
							break
						}
					}
	...
	if s.cache != nil {
		s.cache.Delete(fmt.Sprintf("%s%s", cache.CacheKeyPrefixJawabanDeteksi, existing.IkasID))
		s.cache.Delete(fmt.Sprintf("%s%s", cache.CacheKeyPrefixDeteksi, existing.IkasID))
	}

	return uuid, msg, nil
}

func (s *JawabanDeteksiService) Delete(uuid string, userID string, userRole string, userPerusahaanID string) error {
	if uuid == "" {
		return errors.New("format ID tidak valid")
	}
