...
func (r *KuisRepository) Create(k *models.Kuis) error {
	_, err := r.db.Exec(
		`INSERT INTO kuis (id, id_kelas, id_materi, judul, deskripsi, durasi_menit, passing_grade, is_final, urutan, max_attempt, created_at, updated_at)
		 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`,
		k.ID, k.IDKelas, k.IDMateri, k.Judul, k.Deskripsi, k.DurasiMenit, k.PassingGrade, k.IsFinal, k.Urutan, k.MaxAttempt,
	)
	return err
}
...
func (r *KuisRepository) FindByID(id string) (*models.Kuis, error) {
	row := r.db.QueryRow(
		`SELECT id, id_kelas, id_materi, judul, deskripsi, durasi_menit, passing_grade, is_final, urutan, max_attempt, created_at, updated_at
...
func (r *KuisRepository) Update(k *models.Kuis) error {
	_, err := r.db.Exec(
		`UPDATE kuis SET judul=?, deskripsi=?, durasi_menit=?, passing_grade=?, is_final=?, urutan=?, max_attempt=?, updated_at=NOW()
		 WHERE id=?`,
		k.Judul, k.Deskripsi, k.DurasiMenit, k.PassingGrade, k.IsFinal, k.Urutan, k.MaxAttempt, k.ID,
	)
	return err
}
...
func scanKuis(row *sql.Row) (*models.Kuis, error) {
	...
	err := row.Scan(
		...
		&k.PassingGrade, &k.IsFinal, &k.Urutan, &k.MaxAttempt, &k.CreatedAt, &k.UpdatedAt,
	)
	if err != nil {
		return nil, err
	}
